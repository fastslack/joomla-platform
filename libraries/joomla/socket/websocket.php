...
// Concrete classes.
JLoader::register('JWebSocketUser', __DIR__.'/websocket/user.php');

// Interfaces.
JLoader::register('JWebSocketController', __DIR__.'/websocket/controller.php');
JLoader::register('JWebSocketHandshake', __DIR__.'/websocket/handshake.php');
JLoader::register('JWebSocketProtocol', __DIR__.'/websocket/protocol.php');

// Adapters.
JLoader::discover('JWebSocketHandshake', __DIR__.'/websocket/handshakes');
JLoader::discover('JWebSocketProtocol', __DIR__.'/websocket/protocols');
 ...
 */
class JWebSocket extends JSocketDaemon
{
	private static $_instance;

	public $_controller;

	protected function __construct(JWebSocketController $controller)
	{
		parent::__construct();
	...
	public static function getInstance(JWebSocketController $controller)
	{
		if (!(self::$_instance instanceof JWebSocket))
		{
			self::$_instance = new JWebSocket($controller);
		}
	...
	protected function doHandshake($socket, JWebSocketUser $user)
	{
		...
		if ($bytes == 0)
		{
			JWebSocket::getInstance()->disconnect($socket);
	...
	private function sendFatalErrorResponse()
	{
		...
		if (!$user->handshakeDone())
		{
			JWebSocket::getInstance()->disconnect($user->socket());
			return;
		}
		else
		{
